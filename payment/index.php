...
if (isset($_SESSION['user'])!='' & isset($_GET['product']) & isset($_SESSION['regID']) & isset($_SESSION['password'])) {
	if (!preg_match("/^\w{6}\d{1}\w{1}\d{2}-\d{5}$/", $_SESSION['regID']) || !preg_match("/^\d{1}(\d{1})?$/", $_GET['product']) ) {
...
<section>
	...
	<form action="<?php echo $action; ?>" method="post" name="payuForm">
	  ...
	  <input type="hidden" name="service_provider" value="payu_paisa" size="64" />
          <?php if(!$hash) { ?>
          	<button class="bttn-pill bttn-md bttn-primary" />MAKE PAYMENT</button>
          <?php } ?>
    </form>
