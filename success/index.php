				...
				$query3 = mysqli_query($dbc, $ql3);
				 ...
				 <html>
					...
					<body style="font-family: Verdana, Geneva, sans-serif; background-color: #fffffc; width: 100%; height: 100%">
						...
						<br>
						<h2>Dear '.$name.'</h2>
						<h1>Successfully Registerd to SPE INDHAN 2K18</h1>
...
$query = mysqli_query($dbc, "SELECT * FROM usercredentials WHERE RegID = '$regID' AND email = '$email'");
				...
				if ($eventRow['PAID'] == '1') {
					...
					<body style="font-family: Verdana, Geneva, sans-serif; background-color: #fffffc; width: 100%; height: 100%">
						...
						<br>
						<h2>Dear '.$name.'</h2>
						<h1>Successfully Registerd to '.$productinfo.'</h1>
						<h2>Your Registration ID </h2>
						<p>&nbsp;'.$regID.'</p>
						<h2>Your Transaction ID</h2>
						<p>&nbsp;'.$payumoneyID.'</p>
					...
					$query = mysqli_query($dbc, "SELECT * FROM $productinfo WHERE RegID = '$regID'");
					if ($productinfo == 'appe_workshop' || $productinfo == 'arpt_workshop' || $productinfo == 'ogp_workshop') {
						$query2 = mysqli_query($dbc, "SELECT * FROM werkstatt WHERE RegID = '$regID'");
						$data = mysqli_fetch_assoc($query2);
						if ($data['PAID'] == '1' && $data['attending'] == '1') {
						    $query3 = mysqli_query($dbc, "UPDATE werkstatt SET attending = '2' WHERE RegID = '$regID'");
						} else {
						    $query4 = mysqli_query($dbc, "INSERT INTO werkstatt (RegID, txnid, PAID, workshop, attending) VALUES ('$regID', '$payumoneyID', '1', '$productinfo', '1')");
						}	
						if ($productinfo == 'appe_workshop') $productinfo = 'Advances in Petroleum Production Engineering Workshop';
						elseif ($productinfo == 'arpt_workshop') $productinfo = 'Advances in Refinery Process Technologies Workshop';
						elseif ($productinfo == 'ogp_workshop') $productinfo = 'Oil and Gas Pipelines Workshop';
					}
					$eventRow = mysqli_fetch_assoc($query);
$query = mysqli_query($dbc, "SELECT * FROM usercredentials WHERE RegID = '$regID' AND email = '$email'");
						$rowuser = mysqli_fetch_assoc($query);
          ...
          echo "<h4>We have received a payment of Rs. " . $amount . ". Your order will soon be shipped.</h4>";
			...
			// echo "<h1> Something wrong with your session variable please note these details and mail us</h1>";
	   	    $query = mysqli_query($dbc,"SELECT * FROM usercredentials WHERE email = '$email' AND firstName = '$firstname'");
	   	    $query2 = mysqli_query($dbc, "SELECT * FROM userlogin WHERE email = '$email'");
	   	    $count = mysqli_num_rows($query);
	   	    $count1 = mysqli_num_rows($query2);
	   	    $data = mysqli_fetch_assoc($query);
	   	    $data1 = mysqli_fetch_assoc($query2);
	   	    if ($query && $query2 && ($count == 1) && ($count1 == 1)) {
	   	    	$regID = $data1['RegID'];
	   	    	if (strtoupper($productinfo) == 'REGISTRATION') {
				$ql1 = "UPDATE userlogin SET PAID = '1' WHERE email = '$email' LIMIT 1";
				$ql2 = "UPDATE usercredentials SET PAID = '1' WHERE email = '$email' LIMIT 1";
				...
				$query3 = mysqli_query($dbc, $ql3);
					if ($query1 & $query2 & $query3) {
						$query = mysqli_query($dbc, "SELECT * FROM usercredentials WHERE email = '$email'");
						$rowuser = mysqli_fetch_assoc($query);
						$name = $data['firstName'];
						echo "Successfully Registerd a mail will be sent with registration ID and Transaction ID please check";
						$from = 'admin@speindhan2k18.org';
